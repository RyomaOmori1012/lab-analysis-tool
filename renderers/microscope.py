    ...
    if config['num_targets'] == 1:
        if config['label_style'] == "1段 ＋ 系列名（凡例）":
            ...
            for low in unique_low:
                xs_t = [x_coords_multi[0][internal_ids[i]] for i, l in enumerate(lower_labels) if l == low]
                if xs_t: 
            ...
            for low in unique_low:
                xs = [x_coords_multi[0][internal_ids[i]] for i, l in enumerate(lower_labels) if l == low]
            ...
            xs_up = [x_coords_multi[0][uid] for uid in internal_ids]
            ...
            for i, uid in enumerate(internal_ids):
                ax.text(x_coords_multi[0][uid], y_up, upper_labels[i], ha='center', va=va_up, rotation=rot_up, transform=trans, fontsize=x_lbl_fs, color='black', fontweight='bold', fontname=get_font(upper_labels[i]))
    ...
    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
        ...
        for j in range(config['num_targets']):
            for u1, u2, p in p_pairs_multi[j]:
                ...
                stat_data.append({"ターゲット名": config['target_names'][j] if config['num_targets'] > 1 else "Target", "比較": f"{c1} vs {c2}", "p値": p if not np.isnan(p) else "N/A", "判定": "***" if p<0.001 else "**" if p<0.01 else "*" if p<0.05 else "ns" if not np.isnan(p) else "N/A"})
