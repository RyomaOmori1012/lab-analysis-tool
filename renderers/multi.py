...
def draw_x_labels(ax, config, internal_ids, upper_labels, lower_labels, x_coords_multi, fw, x_span, bar_width):
    ...
    offset_up_user, offset_low_user, offset_tgt_user = config.get('offset_up', 0.0), config.get('offset_low', 0.0), config.get('offset_tgt', 0.0)
    ...
    if config['label_style'] == "1段 ＋ 系列名（凡例）":
        ...
        for j in range(config['num_targets']):
            for low in unique_low:
                ...
                if xs_t: all_xs_low.append(sum(xs_t)/len(xs_t)); all_lbls_low.append(low)
                    
        rot = calc_rotation(all_lbls_low, all_xs_low, x_lbl_fs, fw, x_span)
        y_pos_base, va_val, extra_margin = -0.015 * x_fs_ratio, 'top', 0
        if rot > 0:
        ...
        y_up_base, va_up, extra_margin_up = -0.015 * x_fs_ratio, 'top', 0
        ...
        else: global_lowest_y_base = y_line_base
...
def render_multi_target(input_data, config):
    ...
    raw_target_data_multi, raw_loading_data_multi = {j: {} for j in range(config['num_targets'])}, {j: {} for j in range(config['num_targets'])}
    ...
    p_pairs_multi, test_desc_flat = {j: [] for j in range(config['num_targets'])}, ""
    ...
    for j in range(config['num_targets']):
        for grp in groupings:
            ...
            for u in grp:
                ...
                if len(non_nan_data) >= 2: valid_uids.append(u); valid_data.append(non_nan_data)
                else: dropped_warnings.add(f"{config['target_names'][j]}の{upper_labels[internal_ids.index(u)]}")
    ...
    if config['color_mode'] == "色分け":
        ...
        colors = [cmap(0.2)] if len(unique_up) == 1 else [cmap(i) for i in np.linspace(0.1, 0.6, len(unique_up))]
        palette = {u: colors[i] for i, u in enumerate(unique_up)}
    else: palette = {u: "black" for u in unique_up}
    ...
    for j in range(config['num_targets']):
        ...
        for i, uid in enumerate(internal_ids):
            ...
            if i < len(internal_ids) - 1: current_x += bar_width + bar_gap
    ...
    if x_vals:
        ...
        x_min_val, x_max_val = min(x_vals) - max_gap, current_x - group_gap + max_gap
    ...
    max_element_y, plotted_stars = draw_significance_brackets(ax, sig_pairs_all, current_max_y, base_fs)
    ...
    with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
        ...
        for j in range(config['num_targets']):
            for i, uid in enumerate(internal_ids):
                ...
                t_list, l_list = raw_target_data_multi[j].get(uid, []), raw_loading_data_multi[j].get(uid, [])
                proc_list, norm_list = raw_processed_multi[j].get(uid, []), final_norm_multi[j].get(uid, [])
        ...
        for j in range(config['num_targets']):
            for u1, u2, p in p_pairs_multi[j]:
                ...
                stat_data.append({
                    "ターゲット名": config['target_names'][j], "比較": f"{c1} vs {c2}", 
